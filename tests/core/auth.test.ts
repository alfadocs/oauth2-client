...
function createStorageMock(): AuthStorage {
  ...
  return {
    ...
    async createSession(userId: string): Promise<StoredSession> {
      const session = { userId, cookieValue: `cookie-${userId}` };
...
describe("createAlfadocsAuth", () => {
  it("routes session requests and returns authenticated user", async () => {
    ...
    const user = await storage.createUser("u-1", "john", { id: "u-1", email: "john@example.com" });
    ...
    const req = new Request("https://fn.example/auth", {
    ...
    await expect(res.json()).resolves.toEqual({
      authenticated: true,
      user: { id: "u-1", email: "john@example.com" },
    });
  });
  ...
  it("completes callback and sets session + clears preauth cookie", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.endsWith("/api/v1/me")) {
        return new Response(JSON.stringify({ id: "u-2", email: "mary@example.com" }), {
          status: 200,
        });
      }
  ...
  it("sends patient:list on authorize redirect and on token exchange by default", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.endsWith("/api/v1/me")) {
        return new Response(JSON.stringify({ id: "u-3", email: "x@y.com" }), { status: 200 });
      }
      return new Response("not found", { status: 404 });
    });
    vi.stubGlobal("fetch", fetchMock);
    ...
    expect(new URL(loc).searchParams.get("oauth_token_scopes")).toBe("['patient:list']");
    ...
    expect(tp.get("oauth_token_scopes")).toBe("['patient:list']");
